# copy of https://github.com/tikv/tikv/blob/7fb3666428168327216488ec33c4af28530ab77e/metrics/grafana/common.py
...
def timeseries_panel_histogram(
    title: str, metric: str, unit: str = UNITS.SECONDS
) -> TimeSeries:
    """
    formats metrics which has median, percentile_95, percentile_99, average, max,
     standard_deviation postfixes
    ...
    return timeseries_panel(
        ...
        targets=[
            target(
                expr=expr_simple(f"{metric}_median"),
                legend_format="median",
            ),
            target(
                expr=expr_simple(f"{metric}_percentile_95"),
                legend_format="percentile_95",
            ),
            target(
                expr=expr_simple(f"{metric}_percentile_99"),
                legend_format="percentile_99",
            ),
            target(
                expr=expr_simple(f"{metric}_average"),
                legend_format="average",
            ),
            target(
                expr=expr_simple(f"{metric}_max"),
                legend_format="max",
            ),
            target(
                expr=expr_simple(f"{metric}_standard_deviation"),
                legend_format="standard_deviation",
            ),
        ],
        unit=unit,
        legend_calcs=[
